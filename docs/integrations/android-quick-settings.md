...
Once a tile has been added, the state, label and icon will update to reflect the entity's state and tile settings. When you select a tile you will see the tile momentarily light up as the app calls the server. If successful, the tile will go back to show the entity's state. If there is a failure, the tile changes to a disabled state and an error message will be shown.
