...
The sensor values, location, and other information the app collects are sent to _your_ Home Assistant instance, the same instance you log in to. If you connect the app to more than one instance, you decide for each instance, what information it receives:
...
To set up an encrypted HTTPS connection to your instance, refer to the documentation on [Remote access to Home Assistant](https://www.home-assistant.io/docs/configuration/remote/). The simplest option is to use [Home Assistant Cloud](https://www.home-assistant.io/cloud/), which gives you an encrypted address without any certificate setup.
...
- ![Android](/assets/android.svg) The `full` flavor has no in-app way to opt out of Firebase. You can still reduce its use, though: if you [set up Local Push](../notifications/local.md), your instance delivers notifications directly over a WebSocket connection, and Firebase only steps in as a fallback when that connection is unavailable. To avoid Google entirely, use the `minimal` flavor (from F-Droid or GitHub), which always uses Local Push and never Firebase.
...
Because the app only sends data to your own Home Assistant instance, you can stop sending data and delete what your instance has already stored at any time. Crash reports and notification delivery are handled separately. See [Notifications](#notifications) and [Crash reporting and diagnostics](#crash-reporting-and-diagnostics).
