...
:::info 
<span class='beta'>BETA</span><br /> Google has a hard limit of 100 geofences per app. When combined with High Accuracy mode [zone with trigger range constraints](/docs/core/location#zone-when-using-the-high-accuracy-mode-trigger-range-for-zone-meters-option-value-greater-than-0) you will need to double the count per zone included in the constraint. For example if you have 2 zones total and only want the trigger range for 1 zone that means there will be 3 geofences created.
:::
