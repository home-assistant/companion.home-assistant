...
You can also force the notification to play from the alarm stream so it will make the device ring even if on vibrate/silent ringer mode. On Android 7, use the `channel: alarm_stream` option below to override the default notification sound behavior. To make a notification show up immediately and make a sound regardless of ringer mode, follow one of the examples below.
