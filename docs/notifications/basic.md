...
The **Mobile App** integration will add a notify entity for your device. To send a notification you can use the [**Notifications: Send a notification message** (`notify.send_message`)](https://www.home-assistant.io/actions/notify.send_message/) action.

The notify entity only supports message and title. For more customizable notifications, you can use the classic [`mobile_app` notify platform](#advanced-notifications) instead.
...
6. Under **Targets**, choose what devices you want to notify:
    ...
    - To notify every device in a room, select an area.
    - To notify every device on a floor, select a floor.
