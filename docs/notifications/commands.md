...
<span class='beta'>BETA</span> When the app is already open and showing the targeted server (Home Assistant 2025.6 or later), the command acts on the open page in place instead of relaunching the app, so the state of the page is kept. You can also send `command: reload` to reload the page without cached data, which releases the resources the page holds, like camera streams; when the app is not open, `reload` opens it to the homepage instead.
