...
*  Touch Lock - A sensor to indicate if the touch screen is locked. This sensor is also known as Wet Mode on some devices. The sensor will update as soon as it detects a state change. Most devices require a user to hold the crown or power button for 2 seconds to exit this special mode.
