...
Beginning February 2024 [Let's Encrypt has begun its migration over to providing keys that are not cross-signed by the IdenTrust's root CA certificate](https://letsencrypt.org/2023/07/10/cross-sign-expiration.html), instead using their own now widly trusted root CA cerficiate. Older clients that have not received an updated list of trusted certificate authorities will consider newer Let's Encrypt SSL keys as being invalid, which include versons of Android older than 7.1.1. This will cause Chrome or the Companion App to display an SSL error. An easy workaround is to use Firefox to access Home Assistant as it's distributed with its own updated trusted list of certificate authorities. Another work around is to manually install Let’s Encrypt's Active ISRG Root X1 Self-signed PEM (not DST Root X3) into the Android Credential Storage.
