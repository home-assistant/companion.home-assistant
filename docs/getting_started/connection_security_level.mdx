...
If you use a plain text connection using HTTP instead of an encrypted connection using HTTPS, then will have to choose between two security options in the app if you want to use a unencrypted URL to connect to Home Assistant (such as your local IP address for local connection).

:::note
This option are mandatory to set while onboarding a new server. For users that 
didn't set this option yet it is going to be prompt on each start of the app
...
You can specify your Wi-Fi SSID (for example, "GrandmaHome5G") to be considered your home network. To change the setting open the [Settings](https://my.home-assistant.io/redirect/config/) > **Companion app** > **[server name]** anytime you want.

<details>
  <summary>**Android**</summary>

You can also use a VPN or Ethernet access as home network.
</details>
...
### Implication when not in secure state

With this option it is considered insecure to access an HTTP URL when we are not sure that your device is not at home. Because of that when it happens we do prevent the connection to actually be made.
By doing so we are making sure to not send any credentials or data over an insecure network since the connection is not encrypted.

Anything in background is going to stop working, if you open the app you are going to land on a blocking screen. All the widget or other interaction with the application are going to stop updating.

This will automatically resume once the connection is considered secure again, hence it is important to keep the location permission enabled if the Wi-Fi SSID is used to determine if you are on a home network.

If the location is disabled or the permission rejected or the home network not properly setup the block screen is still going to be display but with potential actions to take.

<img alt="Home Assistant companion app showing blocking screen on insecure network with most secure option picked" src="/assets/android/block_screen.png" width="400"/>

<img alt="Home Assistant companion app showing blocking screen on insecure network with most secure option picked and missing requirements" src="/assets/android/block_screen_missing_requirements.png" width="400"/>
...
To know more about networking check our [troubleshooting section](/docs/troubleshooting/networking).
...
3. Go to [Settings](https://my.home-assistant.io/redirect/config/) > **Companion app** > **[your server]** > **Internal URL**.
...
This is not an option on Android.
...
<details>
  ...
  You can select the VPN option while setting up your home network in [Settings](https://my.home-assistant.io/redirect/config/) > **Companion app** > **[your server]** > **Home network**.
  
  :::note
  It cannot detect on which VPN it connects only that it is connected to a VPN or not.
  :::
</details>
...
:::note
To prevent this behavior use the option **Most secure**.
:::

### I chose "Less secure" but now want to change it. How?

Go to [Settings](https://my.home-assistant.io/redirect/config/) > **Companion app** > **[your server]** > **Connection security level** to change your preference at any time. To define your home network, go to [Settings](https://my.home-assistant.io/redirect/config/) > **Companion app** > **[your server]** > **Internal URL**.
