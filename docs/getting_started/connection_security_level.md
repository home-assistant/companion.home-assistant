...
If you don’t grant location permission or disable location at the OS level but still need to access a non-encrypted URL (for example, http://homeassistant.local:8123), you can choose this option. Use it only if absolutely necessary; **it is not recommended**, as it may expose your Home Assistant data to network eavesdropping, especially on public Wi-Fi.
